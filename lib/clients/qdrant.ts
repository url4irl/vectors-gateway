...
class QdrantHttpClient {
  ...
  async getCollections(): Promise<{ collections: Array<{ name: string }>}> {
    ...
    return { collections: result.collections.map(col => ({ name: col.name })) };
  }
  ...
  async createCollection(name: string, body: { vectors: { size: number; distance: "Cosine" | "Euclid" | "Dot" } }): Promise<void> {
  ...
  async upsert<P = any>(name: string, body: { points: Array<{ id: string | number; vector: number[]; payload?: P }> }): Promise<void> {
